...
namespace OjVolunteer.BLL
{
    public partial class UserEnrollService
    {
        ...
        short delDoneAuditing = (short)Model.Enum.DelFlagEnum.Undone;
        //签到
        ...
        public bool SignOut(int aId, List<int> uIdList)
        {
            ...
            Activity activity = DbSession.ActivityDal.GetEntities(u => u.ActivityID == aId && u.Status == delDoneAuditing).FirstOrDefault();
            foreach (var temp in Data)
            {
                temp.UserEnrollActivityEnd = DateTime.Now>activity.ActivityEnd?activity.ActivityEnd:DateTime.Now;
