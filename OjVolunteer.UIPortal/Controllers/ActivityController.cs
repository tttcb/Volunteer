    ...
    public class ActivityController : BaseController
    {
        ...
        public JsonResult Create(Activity activity)
        {
            try
            {
                ...
                {
                    return Json(new { msg = "timefail" }, JsonRequestBehavior.AllowGet);
                }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (LoginOrganize != null)//组织注册
                {
                    ...
                    activity.Status = activity.ActivityTypeID == 1 ? (LoginOrganize.OrganizeInfoManageId == null ? delUndone : delUndone) : delUndone;//让团队能直接发布活动
        ...
        #region 义工地图
        [LoginCheckFilter(BoolCheckLogin = false)]
        public ActionResult MarkerMap()
        {

            ViewBag.ActivityAddressList = ActivityService.GetEntities(u=>u.ActivityAddress != null&& (u.Status == delNormal || u.Status == delUndone)).ToList();
            
            //var Duration = UserDurationService.GetEntities(u => u.Status==delNormal);
            //UserDurationService.GetEntities(u => u.UserDurationID != 0);
            //Console.Write(UserDurationService.GetEntities(u => u.Status == delNormal));
            //foreach (var item in Duration)
            //{
            //    ViewBag.TotalTime += item.UserDurationTotal;
            //}
            return View();
        }
        [LoginCheckFilter(BoolCheckLogin = false)]
        public ActionResult HeatMap()
        {
            ...
            ViewBag.ActivityAddressList = ActivityService.GetEntities(u => u.ActivityAddress != null && u.Status == delNormal).ToList();
        ...
        #region 日历
        public ActionResult ActivityDate()
        {
            return View();
        }
        public JsonResult GetListData1()
        {
            int pageSize = int.Parse(Request["pageSize"] ?? "5");
            int pageIndex = int.Parse(Request["pageIndex"] ?? "1");
            var PageData = GetActivityData1();
            if (!String.IsNullOrEmpty(Request["typeId"]))
            {
                int typeId = int.Parse(Request["typeId"]);
                PageData = PageData.Where(u => u.ActivityTypeID == typeId).AsQueryable();
        ...
        /// <param name="pageIndex"></param>
        /// <returns></returns>
        public IQueryable<Activity> GetActivityData1()
        {
            var test = Convert.ToDateTime(Request["date"]);
            var testend = test.AddDays(1);
            var DataPage = ActivityService.GetEntities(u => (u.Status == delNormal|| u.Status == delUndone || u.Status == delDoneAuditing )&& u.ActivityEnd<=testend&&u.ActivityStart>=test).AsQueryable();
            return DataPage;
        }
